...
if(isset($_POST)){
    ...
    if(zipData($path,$folder_name,$no_zip)){
        ...
        $status = 1;
        // var_dump($size,$status,$folder_name);
        ...
        // return data to the user
            echo json_encode(["res" => 1111,"filename" => $folder_name,"filesize" => $size, "affectRows" => $final]);
        }
    }else{
        echo json_encode(["res" => 2222]);
    }
    }
    if($_POST["all_backups"]){
        $conn = db();
         // get all the existing data
        $usr_data = $conn->prepare("SELECT * FROM sys_back ORDER BY date_added DESC LIMIT 5");
        $usr_data->setFetchMode(PDO::FETCH_ASSOC);
        $usr_data->execute();
        $data = $usr_data->fetchAll();
        // return all data 
